...
template<class T>
MapPointTmpl<T>::MapPointTmpl<T> (DocumentImpl* pDoc)
...
template<class T>
MapPointTmpl<T>::~MapPointTmpl<T> ()
{
}


template <class T>
bool MapPointTmpl<T>::getObjectId(const String*& pId) const
{
    if (m_bName_valueSet)
    {
        pId = &m_Name;
...
template <class T>
ValueObject* MapPointTmpl<T>::id() const
{
    if (m_bName_valueSet)
    {
...
template <class T>
void MapPointTmpl<T>::toXml (IStream& stream)
{
    static const wchar_t* kstrElementName = L"MapPoint";
    stream.write(L"<");
...
template <class T>
Object::ValidityEnum MapPointTmpl<T>::validate (IValidationEventSink* pEventSink) const
{
