...
template<class T>
LocationTmpl<T>::LocationTmpl<T> (DocumentImpl* pDoc)
...
template<class T>
LocationTmpl<T>::~LocationTmpl<T> ()
{
}


template <class T>
bool LocationTmpl<T>::getObjectId(const String*& pId) const
{
    if (m_bName_valueSet)
    {
        pId = &m_Name;
...
template <class T>
ValueObject* LocationTmpl<T>::id() const
{
    if (m_bName_valueSet)
    {
...
template <class T>
void LocationTmpl<T>::toXml (IStream& stream)
{
    static const wchar_t* kstrElementName = L"Location";
    stream.write(L"<");
...
template <class T>
Object::ValidityEnum LocationTmpl<T>::validate (IValidationEventSink* pEventSink) const
{
