...
template<class T>
CenterTmpl<T>::CenterTmpl<T> (DocumentImpl* pDoc)
...
template<class T>
CenterTmpl<T>::~CenterTmpl<T> ()
{
}


template <class T>
bool CenterTmpl<T>::getObjectId(const String*& pId) const
{
    if (m_bName_valueSet)
    {
        pId = &m_Name;
...
template <class T>
ValueObject* CenterTmpl<T>::id() const
{
    if (m_bName_valueSet)
    {
...
template <class T>
void CenterTmpl<T>::toXml (IStream& stream)
{
    static const wchar_t* kstrElementName = L"Center";
    stream.write(L"<");
...
template <class T>
CgPoint* CenterTmpl<T>::resolvePntRef ()
{
    if (m_bPntRef_valueSet)
    {
...
template <class T>
const DoubleCollection& CenterTmpl<T>::resolveCoor () const
{
    if (value().size() > 0)
    {
        return value();
...
template <class T>
Object::ValidityEnum CenterTmpl<T>::validate (IValidationEventSink* pEventSink) const
{
