...
template<class T>
InstrumentPointTmpl<T>::InstrumentPointTmpl<T> (DocumentImpl* pDoc)
...
template<class T>
InstrumentPointTmpl<T>::~InstrumentPointTmpl<T> ()
{
}


template <class T>
bool InstrumentPointTmpl<T>::getObjectId(const String*& pId) const
{
    if (m_bName_valueSet)
    {
        pId = &m_Name;
...
template <class T>
ValueObject* InstrumentPointTmpl<T>::id() const
{
    if (m_bName_valueSet)
    {
...
template <class T>
void InstrumentPointTmpl<T>::toXml (IStream& stream)
{
    static const wchar_t* kstrElementName = L"InstrumentPoint";
    stream.write(L"<");
...
template <class T>
Object::ValidityEnum InstrumentPointTmpl<T>::validate (IValidationEventSink* pEventSink) const
{
