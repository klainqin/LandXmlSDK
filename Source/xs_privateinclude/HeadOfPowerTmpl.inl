...
template<class T>
HeadOfPowerTmpl<T>::HeadOfPowerTmpl<T> (DocumentImpl* pDoc)
    : ObjectTmpl<T>(pDoc)
{
    m_Name = L"";
...
template<class T>
HeadOfPowerTmpl<T>::~HeadOfPowerTmpl<T> ()
{
}


template <class T>
bool HeadOfPowerTmpl<T>::getObjectId(const String*& pId) const
{
    if (m_bName_valueSet)
    {
        pId = &m_Name;
...
template <class T>
ValueObject* HeadOfPowerTmpl<T>::id() const
{
    if (m_bName_valueSet)
    {
...
template <class T>
String HeadOfPowerTmpl<T>::getName() const
{
    return m_Name;
}

template <class T>
void HeadOfPowerTmpl<T>::setName(String value)
{
...
template <class T>
bool HeadOfPowerTmpl<T>::hasValue_Name() const
{
    return m_bName_valueSet;
}

template <class T>
void HeadOfPowerTmpl<T>::resetValue_Name()
{
    m_bName_valueSet = false;
}
...
template <class T>
void HeadOfPowerTmpl<T>::toXml (IStream& stream)
{
    static const wchar_t* kstrElementName = L"HeadOfPower";
    stream.write(L"<");
...
template <class T>
Object::ValidityEnum HeadOfPowerTmpl<T>::validate (IValidationEventSink* pEventSink) const
{
